...
public class AllStationJDToEs2 {

	private static Logger logger = LoggerFactory.getLogger(AllStationJDToEs2.class);
	...
	public static void writeToES() {
		...
		try {
			...
			FileStatus[] files = fs.listStatus(new Path("/gome/nrdmbs/logs/gomeIdentify_beta2/" + yesterday));
			for (FileStatus fileStatus : files) {
				...
				while ((line = br.readLine()) != null) {
					...
					// json.put("jdcatNameInfo", jdcatNameInfo);
					...
					json.remove("gome");
					json.put("gomedetail", detail);

					builder.add(esClient.prepareIndex(esIndex, esType).setSource(json.toString()));
				}
